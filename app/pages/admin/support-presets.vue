...
async function loadPresets() {
  loading.value = true
  presets.value = await $fetch<any[]>('/api/support-preset/list').catch(() => [])
  loading.value = false
}
...
async function submitForm() {
  ...
  try {
    if (isCreating.value) {
      const res = await $fetch<any>('/api/support-preset/create', {
        method: 'POST',
        body: payload,
      })
      ...
      await $fetch(`/api/support-preset/${selectedId.value}`, {
        method: 'PUT',
        body: payload,
      })
...
async function deletePreset() {
  ...
  try {
    await $fetch(`/api/support-preset/${selectedId.value}`, { method: 'DELETE' })
