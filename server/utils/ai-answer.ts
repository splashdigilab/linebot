...
export type MessageIntent = 'greeting' | 'thanks' | 'farewell' | 'find_human' | 'sensitive' | 'compare' | 'list' | 'question'
...
const VALID_INTENTS: MessageIntent[] = ['greeting', 'thanks', 'farewell', 'find_human', 'sensitive', 'compare', 'list', 'question']
...
回傳 JSON：{ "intent": "...", "isFollowup": true/false, "standaloneQuery": "...", "compareItems": [] }`
...
export async function classifyIntent(text: string, history?: AiChatTurn[]): Promise<IntentResult | null> {
  ...
  try {
    const { data, inputTokens, outputTokens } = await generateJson<{ intent?: unknown; isFollowup?: unknown; standaloneQuery?: unknown; compareItems?: unknown }>(prompt, {
      systemInstruction: INTENT_SYSTEM_INSTRUCTION,
      temperature: 0,
      // 多吐改寫問題 + compareItems，maxOutputTokens 從 200 拉到 280 留餘裕
      maxOutputTokens: 280,
...
export async function answerWithAi(input: AnswerInput): Promise<AnswerOutput> {
  ...
  if (!input.skipDisambiguation && !isCompare && !isList && shouldDisambiguate(dedupedChunks, settings)) {
