...
回傳 JSON：{ "intent": "...", "isFollowup": true/false, "standaloneQuery": "..." }`
...
export async function classifyIntent(text: string, history?: AiChatTurn[]): Promise<IntentResult | null> {
  ...
  try {
    const { data, inputTokens, outputTokens } = await generateJson<{ intent?: unknown; isFollowup?: unknown; standaloneQuery?: unknown }>(prompt, {
      systemInstruction: INTENT_SYSTEM_INSTRUCTION,
      temperature: 0,
      // 多吐一句改寫問題，maxOutputTokens 從 80 拉到 200 留餘裕
      maxOutputTokens: 200,
...
export async function answerWithAi(input: AnswerInput): Promise<AnswerOutput> {
  ...
  if (contextQuery && (looksLikeFollowup || replyingToClarification || topSimilarity < getGroundingThreshold(settings))) {
    try {
